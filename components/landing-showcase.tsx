...
/**
 * The "Ask MARINA" floating chat dock — the product's USP. Frozen at a
 * realistic Q&A so a visitor instantly grasps "I can ask anything about
 * anyone and get a grounded answer in seconds."
 */
export function AskMarinaMockup() {
  return (
    <MockupShell label="Ask MARINA · Sneha P.">
      <div className="p-5 pt-3 bg-[var(--m-bg-soft)]/40 min-h-[420px]">
        {/* Header strip — who the chat is about */}
        <div className="flex items-center gap-2.5 mb-4 px-3 py-2 rounded-xl bg-white border border-[var(--m-border-soft)]">
          <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-[var(--m-clay-soft)] text-[11px] font-semibold text-[var(--m-clay-deep)]">
            SP
          </span>
          <div className="flex-1 min-w-0">
            <p className="text-[12.5px] font-semibold text-[var(--m-ink)] leading-tight">Sneha Patel</p>
            <p className="text-[10.5px] text-[var(--m-ink-3)]">Senior Engineer · Billing</p>
          </div>
          <span className="text-[10px] uppercase tracking-wider text-[var(--m-accent)] font-semibold">
            Grounded
          </span>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-end">
            <div className="max-w-[80%] px-3 py-2 rounded-2xl rounded-br-md bg-[var(--m-accent)] text-white text-[12.5px] leading-relaxed shadow-sm">
              What did Sneha ship this week?
            </div>
          </div>

          {/* A1 with citation chips */}
          <div className="flex justify-start">
            <div className="max-w-[90%]">
              <div className="px-3 py-2.5 rounded-2xl rounded-bl-md bg-white border border-[var(--m-border-soft)] text-[12.5px] text-[var(--m-ink)] leading-relaxed shadow-sm">
                Three PRs in <span className="font-medium text-[var(--m-accent-2)]">acme/billing</span>: auth
                refactor, webhook retry, dunning emails. Spent ~6h pairing with Arjun on the Razorpay
                integration. One blocker on Wed (Stripe key rotation) — cleared in 47 min.
              </div>
              <div className="mt-1.5 flex flex-wrap gap-1">
                <CitationChip>3 commits</CitationChip>
                <CitationChip>1 pair session</CitationChip>
                <CitationChip>1 blocker · resolved</CitationChip>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-start">
            <div className="max-w-[90%]">
              <div className="px-3 py-2.5 rounded-2xl rounded-bl-md bg-white border border-[var(--m-border-soft)] text-[12.5px] text-[var(--m-ink)] leading-relaxed shadow-sm">
                Yes — soft signal. Logged 51h this week (avg is 38h), took no full lunch break Tue–Thu,
                and her last leave was 73 days ago. Recommend a check-in.
              </div>
              <div className="mt-1.5 flex flex-wrap gap-1">
                <CitationChip tone="warn">51h / wk</CitationChip>
                <CitationChip tone="warn">0 lunch breaks · 3 days</CitationChip>
                <CitationChip tone="warn">73d since leave</CitationChip>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Input bar — mock */}
        <div className="mt-4 flex items-center gap-2 px-3 py-2 rounded-xl bg-white border border-[var(--m-accent)]/30 shadow-sm">
          <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-[var(--m-accent-soft)] text-[var(--m-accent-2)]">
            <svg width={11} height={11} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
              <path d="M4 4h16v12H6l-2 2V4z" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </span>
          <p className="flex-1 text-[11.5px] text-[var(--m-ink-3)]">Ask anything about Sneha…</p>
          <span className="text-[10px] px-2 py-0.5 rounded-md bg-[var(--m-ink)] text-white font-medium">
            Ask
          </span>
        </div>
      </div>
...
/**
 * AI Daily Brief — the autogenerated employee story / weekly CEO digest.
 * Shows that MARINA writes prose, not dashboards.
 */
export function AiBriefMockup() {
  return (
    <MockupShell label="AI Daily Brief · Mon Jun 9">
      <div className="p-5 pt-3 min-h-[420px]">
        {/* Eyebrow + AI tag */}
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-[10px] tracking-wider uppercase text-[var(--m-ink-4)]">Your Monday brief</p>
            <p className="font-display text-[20px] leading-tight text-[var(--m-ink)] mt-0.5">
              The 4-minute team report.
            </p>
          </div>
          <span className="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider font-semibold text-[var(--m-clay-deep)] bg-[var(--m-clay-soft)] px-2 py-0.5 rounded-full">
            <SparkleIcon /> AI-written
          </span>
        </div>

        {/* Highlight block — what shipped */}
        <div className="rounded-xl border border-[var(--m-accent)]/25 bg-[var(--m-accent-soft)]/40 p-3 mb-2.5">
          <p className="text-[10.5px] uppercase tracking-wider text-[var(--m-accent-2)] font-semibold mb-1">
            What shipped
          </p>
          <p className="text-[12.5px] text-[var(--m-ink)] leading-relaxed">
            The team shipped <span className="font-semibold">11 deliverables</span> last week — 7 from
            engineering, 3 from design, 1 from sales. Highlights: <span className="font-medium">Razorpay
            v2</span> went live, the <span className="font-medium">onboarding redesign</span> hit staging,
            and Arjun closed the <span className="font-medium">Vembu deal (₹12L ARR)</span>.
          </p>
        </div>

        {/* Risk block */}
        <div className="rounded-xl border border-[var(--m-bad)]/25 bg-[var(--m-bad-soft)]/40 p-3 mb-2.5">
          <p className="text-[10.5px] uppercase tracking-wider text-[var(--m-bad)] font-semibold mb-1">
            Who needs attention
          </p>
          <ul className="space-y-1 text-[12.5px] text-[var(--m-ink)] leading-relaxed">
            <li>
              · <span className="font-medium">Sneha</span> — 51h logged, no leave in 73d. Burnout watch.
            </li>
            <li>
              · <span className="font-medium">Rahul</span> — blocked 2 days on design handoff from Priya.
            </li>
            <li>
              · <span className="font-medium">Design pod</span> — 4 of 6 deliverables slipping past Friday.
            </li>
          </ul>
        </div>

        {/* Recommendation block */}
        <div className="rounded-xl border border-[var(--m-gold)]/30 bg-[var(--m-gold-soft)]/30 p-3">
          <p className="text-[10.5px] uppercase tracking-wider text-[var(--m-gold)] font-semibold mb-1">
            MARINA recommends
          </p>
          <ul className="space-y-1 text-[12.5px] text-[var(--m-ink)] leading-relaxed">
            <li>1. 15-min check-in with Sneha. Suggest she takes Fri off.</li>
            <li>2. Unblock Rahul → route the handoff to Anika (Priya is on leave).</li>
            <li>3. Re-scope the design pod before Wednesday standup.</li>
          </ul>
        </div>

        <p className="mt-3 text-[10px] text-[var(--m-ink-4)] text-center">
          Generated from 1,247 events across GitHub, Calendar, Slack, and MARINA timesheets.
        </p>
      </div>
    </MockupShell>
  )
...
function CitationChip({
  ...
  const cls =
    tone === 'warn'
      ? 'bg-[var(--m-bad-soft)] text-[var(--m-bad)] border-[var(--m-bad)]/20'
      : 'bg-[var(--m-info-soft)] text-[var(--m-info)] border-[var(--m-info)]/20'
  return (
    <span className={`inline-flex items-center text-[9.5px] font-medium px-1.5 py-0.5 rounded border ${cls}`}>
      {children}
    </span>
  )
}
...
function SparkleIcon() {
  return (
    <svg width={9} height={9} viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 2l1.5 6L20 9.5l-6.5 1.5L12 17l-1.5-6L4 9.5 10.5 8z" />
    </svg>
  )
}
