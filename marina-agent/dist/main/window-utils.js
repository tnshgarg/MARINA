...
exports.hideDockIfIdle = hideDockIfIdle;
...
function bringToFront(win) {
    if (process.platform === 'darwin' && electron_1.app.dock?.show) {
...
function hideDockIfIdle() {
    if (process.platform !== 'darwin' || !electron_1.app.dock?.hide)
        return;
    // Defer one tick so the closing window has a chance to clear from BrowserWindow.getAllWindows()
    setTimeout(() => {
        const anyVisible = electron_1.BrowserWindow.getAllWindows().some((w) => w.isVisible() && !w.isDestroyed());
        if (!anyVisible) {
            void electron_1.app.dock.hide();
        }
    }, 50);
}
