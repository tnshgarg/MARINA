...
function scheduleForCurrentHour() {
    if (stopped)
        return;
    ...
    if (msLeft < 60_000)
        return; // negligible — next call from setInterval covers it
...
async function attemptCapture() {
    ...
    try {
        ...
        state_1.bus.patch({ lastError: `capture: ${err.message}` });
        return;
    }
    if (jpeg.byteLength === 0)
        return;
    // Camera-glyph flash — UX disclosure that something just happened.
    (0, tray_1.flashCameraIndicator)();
    try {
        await (0, api_1.authedPost)('/api/agent/screenshots', {
        ...
        state_1.bus.patch({ lastError: `shot upload: ${err.message}` });
    }
}
...
async function captureJpeg() {
    ...
    const composed = electron_1.nativeImage.createFromBuffer(strip, { width: stripWidth, height: stripHeight });
    return composed.toJPEG(70);
}
//# sourceMappingURL=shotter.js.map