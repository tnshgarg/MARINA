...
import { generateSlots, getAvailability, getBusyIntervals } from '@/lib/booking/availability'
...
export default async function BookPage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  // Calendly-style: offer the host's real open slots for the next two weeks.
  const config = await getAvailability(host.id)
  const busy = await getBusyIntervals(host.id, config.slotMin)
  const days = generateSlots(config, busy, { maxDays: 7, scanDays: 21, minLeadMin: 120 })
    ...
    <main className="min-h-screen paper text-[var(--m-ink)] flex items-center justify-center px-6 py-16">
      <div className="w-full max-w-lg">
        ...
        <p className="text-center text-[14px] text-[var(--m-ink-3)] mt-2 mb-6">
          {days.length > 0
            ? `Pick an open slot below — ${name.split(' ')[0]} will confirm.`
            : `Propose a time and ${name.split(' ')[0]} will confirm.`}
        </p>
        <BookingForm handle={host.login} hostName={name} days={days} timezone={config.timezone} slotMin={config.slotMin} />
