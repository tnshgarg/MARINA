...
export async function POST(req: Request, ctx: { params: Promise<{ orgId: string }> }) {
  ...
  try {
    ...
    const body = (await req.json().catch(() => ({}))) as { yesterday?: string; today?: string; blockers?: string }
    ...
    await saveStandup({ orgId, userId: session.appUserId, yesterday, today, blockers, source: 'web' })
