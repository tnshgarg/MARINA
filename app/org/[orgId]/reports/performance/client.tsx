...
export default function PerformanceReportClient({ report }: { report: PerformanceReport }) {
  return (
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between gap-3 flex-wrap">
        <div>
      ...
      <div className="bg-[var(--m-bg)] min-h-screen py-6">
        <article className="report-page max-w-[820px] mx-auto bg-white border border-slate-200 rounded-md shadow-sm p-12 text-slate-900">
          {/* Letterhead */}
          <div className="flex items-baseline justify-between gap-4 pb-5 mb-6 border-b border-slate-200">
            <div>
              <p className="font-display text-[28px] leading-tight text-slate-900">
                Performance review
              </p>
            ...
            <div className="text-right">
              ...
              <p className="text-[11px] text-slate-500">AI Chief of Staff</p>
            </div>
          </div>
          ...
          <section className="mb-6">
            ...
            <p className="text-[13px] text-slate-600 mt-0.5">
              ...
              {report.employee.email && (
                <>
                  {' · '}
                  <span className="text-slate-500">{report.employee.email}</span>
                </>
              )}
                ...
                <tbody>
                  {report.highlights.map((h, i) => (
                    <tr key={i} className="border-b border-slate-100 last:border-0">
                      <td className="py-1.5 pr-3 text-slate-500 tabular-nums w-24">{h.date}</td>
                      <td className="py-1.5 text-slate-800">{h.title}</td>
                      {h.kind && <td className="py-1.5 pl-2 text-slate-400 text-right">{h.kind}</td>}
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <ul className="space-y-1.5">
                {report.blockers.map((b, i) => (
                  <li key={i} className="flex items-start gap-2 text-[12.5px] text-slate-700">
                    <span className="tabular-nums w-24 shrink-0 text-slate-500">{b.date}</span>
                    <span className="text-slate-500 w-14 shrink-0 tabular-nums">{Math.round(b.minutes / 60 * 10) / 10}h</span>
                    <span>{b.reason}</span>
                  </li>
                ))}
              </ul>
          ...
          {/* Recommendation */}
          <section className="mt-8 pt-5 border-t border-slate-200">
            <p className="text-[11px] uppercase tracking-wider font-semibold text-slate-500 mb-1.5">
...
function Stat({
  ...
  return (
    <div className="rounded-md border border-slate-200 px-3 py-2.5">
      ...
      <p className="mt-1.5 text-[10.5px] uppercase tracking-wider font-medium text-slate-500">{label}</p>
    </div>
  )
}
